              ...
              <div class="w-full flex justify-between items-center">
                ...
                <div>
                  <button class="bg-transparent focus:outline-none cursor-text">
                    <span class="text-xl text-green-500 font-black">{{pick}}</span>
                  </button>
                </div>
              </div>
            </div>
...
export default {
  ...
  computed: {
    picks() {
      return this.$store.getters.singleGameArray;
    },
    errorMessage() {
  ...
  methods: {
    selectNumber(number) {
      this.$store.dispatch("selectNumber", number);
    },
    removeNumber(pick) {
      this.$store.dispatch("removeNumber", pick);
    },
    playGame() {
      if (!this.isLoggedIn) {
